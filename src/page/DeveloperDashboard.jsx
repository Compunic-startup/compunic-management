...
function DeveloperDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribeExpenses = onSnapshot(expensesQuery, (snapshot) => {
      setMyExpenses(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
    return () => {
        unsubscribeTokens();
        unsubscribeRequests();
...
const handleResolveSubmit = async (e) => {
    ...
    try {
      ...
      // --- MODIFIED WHATSAPP LOGIC ---
      // The message is prepared, but no specific number is used.
  ...
  const handleSubmitExpense = async (e) => {
    ...
    setExpenseError('');
    setIsSubmitting(true); // Re-use isSubmitting state for loading
    try {
    ...
    <div className="min-h-screen bg-slate-100">
      <header className="bg-white shadow-md sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div>
              <h1 className="text-lg font-bold text-slate-800">Developer Dashboard</h1>
            ...
            <div className="flex items-center space-x-4">
              ...
              <button onClick={() => setIsLeaveModalOpen(true)} className="px-4 py-2 text-sm font-semibold text-white bg-amber-500 rounded-lg shadow-sm hover:bg-amber-600 transition-colors">Apply for Leave</button>
              <button onClick={handleDownloadExcel} className="px-4 py-2 text-sm font-semibold text-white bg-green-500 rounded-lg shadow-sm hover:bg-green-600 transition-colors">Download Report</button>
              <button onClick={handleLogout} className="px-4 py-2 text-sm font-semibold text-white bg-red-500 rounded-lg shadow-sm hover:bg-red-600 transition-colors">Sign Out</button>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm mb-8">
            <h2 className="text-lg font-semibold text-slate-800 mb-4">My Assigned Tokens</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <input type="text" placeholder="Search by Ticket ID or Project..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="p-2 border border-slate-300 rounded-md w-full" />
                ...
                <table className="min-w-full divide-y divide-slate-200">
                    <thead className="bg-slate-50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Raised By</th>
                    ...
                    <tbody className="bg-white divide-y divide-slate-200">
                        ...
                        {!loading && paginatedTokens.map((token) => (
                            <tr key={token.id} onClick={() => handleOpenDetailsModal(token)} className="hover:bg-slate-50 cursor-pointer">
                                <td className="px-6 py-4 font-mono text-sm text-slate-600">{token.ticketId}</td>
                                <td className="px-6 py-4 text-sm text-slate-800 font-medium">{token.projectName}</td>
                                <td className="px-6 py-4 text-sm text-slate-500">{token.raisedBy}</td>
            ...
            {totalPages > 1 && (
                <div className="flex justify-between items-center mt-6">
                    <button onClick={() => setCurrentPage(p => p > 1 ? p - 1 : p)} className="px-4 py-2 text-sm font-medium rounded-md border disabled:opacity-50">Previous</button>
                    <span>Page {currentPage} of {totalPages}</span>
                    <button onClick={() => setCurrentPage(p => p < totalPages ? p + 1 : p)} className="px-4 py-2 text-sm font-medium rounded-md border disabled:opacity-50">Next</button>
                </div>
            )}
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm mb-8">
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                    ...
                    <tbody className="bg-white divide-y divide-slate-200">
                        {loading && <tr><td colSpan="5" className="text-center py-4 text-slate-500">Loading tasks...</td></tr>}
                        {!loading && myTasks.map((task) => (
                            <tr key={task.id}>
                                <td className="px-6 py-4 text-sm text-slate-800 max-w-md truncate">{task.description}</td>
                                <td className="px-6 py-4 text-sm text-slate-500">{task.assignedBy}</td>
                                <td className="px-6 py-4 text-sm font-mono text-slate-600">{task.deadline}</td>
                        ...
                        {!loading && myTasks.length === 0 && (<tr><td colSpan="5" className="text-center py-4 text-slate-500">You have no tasks assigned.</td></tr>)}
                      </tbody>
                </table>
            </div>
        </div>
                ...
                <tbody className="bg-white divide-y divide-slate-200">
                  {myExpenses.map(exp => (
                    <tr key={exp.id}>
                      ...
                      <td className="px-6 py-4 text-sm text-slate-500">{exp.submittedAt.toDate().toLocaleDateString()}</td>
                      <td className="px-6 py-4"><StatusBadge status={exp.status} /></td>
                    </tr>
                  ))}
                  {myExpenses.length === 0 && (<tr><td colSpan="4" className="text-center py-4 text-slate-500">No expenses submitted.</td></tr>)}
                </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-20  bg-opacity-75 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-lg">
                ...
                <form onSubmit={handleResolveSubmit}>
                    <textarea value={resolutionNotes} onChange={(e) => setResolutionNotes(e.target.value)} rows="5" className="mt-1 block w-full p-2 border rounded-md" placeholder="e.g., Fixed the null pointer..."/>
                    {resolutionError && <p className="text-red-500 text-sm mt-2">{resolutionError}</p>}
                    <div className="flex justify-end space-x-4 mt-6">
                        <button type="button" onClick={handleCloseResolveModal} className="px-4 py-2 bg-slate-200 rounded-md">Cancel</button>
